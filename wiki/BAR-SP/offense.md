...
The full animation shows a recurring pattern that appears across most of Barcelona's short goal kicks. The keeper plays a short pass to one of the centre-backs in the box corner, who immediately progresses the ball to the fullback on the same side. From there, Barcelona look for one of two continuations. The first is a combination with the winger or attacking midfielder in the wide corridor, using a give-and-go or a third-man pattern to escape the first pressing wave along the touchline. The second is a switch towards the centre, either through a dribble carry into the (now open) half-space by the fullback, or through a diagonal pass into the dropping central midfielder, who then redirects the build-up to the opposite side of the pitch. In both cases, the goal kick is treated as the first action of a structured possession sequence rather than as an isolated restart.
