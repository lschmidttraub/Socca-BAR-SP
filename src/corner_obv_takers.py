"""Corner kick OBV comparison for FC Barcelona.

Compares OBV-for-net per corner kick for individual takers
(Raphael Dias, Marcus Rashford, Lamine Yamal) against the
tournament-wide average across all corner takers.

Run from the project root:
    python src/corner_obv_takers.py
"""
...
# matches.csv may live at the project root or inside data/
...
OUT = ROOT / "assets" / "corner_analysis" / "corner_obv_takers.png"

# ── Config ───────────────────────────────────────────────────────────────────
TEAM           = "Barcelona"
TARGET_PLAYERS = ["Raphael Dias", "Marcus Rashford", "Lamine Yamal"]  # substring match

# ── Colors (project palette) ─────────────────────────────────────────────────
FOCUS_COLOR   = "#4575b4"   # Barcelona blue
AVG_COLOR     = "#d73027"   # tournament average red
NEUTRAL_COLOR = "#878787"   # grey
...
for target in TARGET_PLAYERS:
    ...
    if vals:
        print(f"  {player_full_names[target]}: {len(vals)} corners, mean OBV = {np.mean(vals):.4f}")
    else:
        print(f"  {target}: no data found")
print(f"  All takers   : {len(all_takers_obv)} corners, mean OBV = {np.mean(all_takers_obv):.4f}")
...
ax.set_title(f"{TEAM} — Corner Kick OBV by Taker",
